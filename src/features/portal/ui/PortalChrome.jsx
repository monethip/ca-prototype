...
function crumbsFor(view) {
  const portal = { id: "certs", label: "Portal" };
...
function Icon({ name }) {
  const common = { width: 22, height: 22, fill: "none", stroke: "currentColor", strokeWidth: 1.8, strokeLinecap: "round", strokeLinejoin: "round" };
  if (name === "certs") {
    return (
              ...
              return (
                <span key={c.label + i}>
                  ...
                  {last || !c.id ? (
                    ...
                    <button type="button" className="crumb-link" onClick={() => onCrumb(c.id)}>
                      {c.label}
                    </button>
                  )}
                </span>
